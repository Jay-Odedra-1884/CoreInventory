    ...
    <div className="flex flex-col rounded-2xl bg-white p-6 shadow-sm border border-gray-50 h-full">
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="text-right">
          <span className="text-[10px] bg-gray-100 text-gray-500 font-bold px-2 py-0.5 rounded-full uppercase tracking-tighter">
            Current Month
          </span>
        </div>
      </div>
        ...
        <div className="relative flex-1 border-b border-gray-100 pb-8">
          ...
          <div className="flex h-full items-end justify-center gap-12 px-4">
            ...
            <div className="relative group flex flex-col items-center w-10">
              <div className="absolute -top-6 opacity-0 group-hover:opacity-100 transition-opacity bg-black text-white text-[10px] px-2 py-0.5 rounded -translate-y-1 font-bold">
                ₹{receipts}
              </div>
              <div
                className="w-full rounded-t-lg transition-all duration-700 ease-out shadow-sm ring-1 ring-inset ring-black/5"
                style={{
                  height: `${rPct}%`,
                  background: "linear-gradient(to top, #fa5252, #f03e3e)",
                }}
              />
            ...
            <div className="relative group flex flex-col items-center w-10">
              ...
              <div
                ...
                style={{
                  height: `${dPct}%`,
                  background: "linear-gradient(to top, #40c057, #37b24d)",
                }}
              />
              <span className="absolute -bottom-6 text-[10px] font-bold text-gray-400">
                DELIVERY
      ...
      <div className="mt-10 flex items-center justify-between pt-5 border-t border-gray-50">
        <div className="flex items-center gap-5">
          ...
          <div className="flex items-center gap-1.5 text-[10px] font-bold text-gray-500 uppercase tracking-wider">
            <span className="h-2.5 w-2.5 rounded-full bg-[#37b24d]" />{" "}
            Deliveries
          </div>
        </div>
        <div
          className={`flex items-center gap-1.5 text-[12px] font-extrabold px-3 py-1 rounded-full border ${net >= 0 ? "text-[#37b24d] bg-green-50 border-green-100" : "text-[#f03e3e] bg-red-50 border-red-100"}`}
        >
    ...
    <div className="w-full bg-[#f8f9fa] text-gray-800 font-sans pb-12">
      ...
      <nav className="bg-white px-8 py-4 flex justify-between items-center shadow-sm sticky top-0 z-40 border-b border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-lg font-bold tracking-tight text-gray-900 underline decoration-[#37b24d] decoration-4 underline-offset-4">
            CoreInventory
          </h1>
        </div>
        <div className="flex gap-4 bg-black/5 p-1 rounded-full transition-all">
          {["Home", "Operations", "Products", "Dashboard", "Logout"].map(
            (item) => (
              <span
                key={item}
      ...
      <div className="mx-auto max-w-[1400px] px-8 py-8">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-5 mb-8">
          <KpiCard
            ...
            sub="Unique items in stock"
          />
          <KpiCard
            label="Critical Alerts"
            value={kpi.low_stock + kpi.out_of_stock}
            sub={`${kpi.out_of_stock} empty · ${kpi.low_stock} low`}
          />
          <KpiCard
            label="Pending In"
            value={kpi.pending_receipts}
            sub="Incoming shipments"
          />
          <KpiCard
            label="Pending Out"
            value={kpi.pending_deliveries}
            sub="Orders to fulfill"
          />
          <KpiCard
            label="Internal"
            value={kpi.internal_transfers}
            sub="Transfer requests"
          />
        </div>
